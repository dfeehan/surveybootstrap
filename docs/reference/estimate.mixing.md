...
- `choose.next.state.fn` a function which can be passed a vector of
  states and will return a draw of a subsequent state for each entry in
  the vector
...
- `states` a list with an entry for each state. within each state's
  ...
  - `trans.fn` a function which, when called, randomly chooses a next
    state with probabilities given by the transition probs.
